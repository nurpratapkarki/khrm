...
import { trainingApi, companyApi, type TrainingCourse, type Office } from '@/api';
import { Card, CardContent } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { ArrowLeft, Clock, CheckCircle2, ListChecks, BookOpen, Award, GraduationCap, MessageCircle } from 'lucide-react';
          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            <div className="space-y-6 max-w-full">
              <Badge variant="secondary" className="bg-white/20 text-white border-white/30 whitespace-normal text-left h-auto py-1">
                ...
                <span className="inline">{course.course_type_display}</span>
              </Badge>

              <h1 className="text-3xl md:text-5xl font-bold break-words hyphens-auto">{course.name}</h1>
              ...
              <div className="flex flex-wrap gap-4">
                <div className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white/10 backdrop-blur-sm border border-white/20 max-w-full">
                  ...
                  <span className="text-sm font-medium break-words whitespace-normal">{course.duration}</span>
                </div>
                {course.certification_provided && (
                  <div className="flex items-center gap-2 px-4 py-2 rounded-lg bg-accent-gold/20 backdrop-blur-sm border border-accent-gold/30 max-w-full">
                    ...
                    <span className="text-sm font-medium break-words whitespace-normal">Certificate Provided</span>
                  </div>
                )}
              </div>
            </div>
              ...
              <Card className="border-2 border-accent-gold/20 bg-linear-to-br from-accent-gold/5 to-transparent">
                <CardContent className="pt-8 pb-8">
                  <div className="text-center space-y-4">
                    ...
                    <div>
                      <h3 className="text-xl font-bold mb-2">Course Details Available on Request</h3>
                      <p className="text-muted-foreground max-w-md mx-auto">
                        Detailed course content, syllabus, and requirements will be shared during your counseling session.
                      </p>
                    </div>
                    {whatsappLink ? (
                      <Button
